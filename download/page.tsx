...
export default function Downloads() {
  ...
  useEffect(() => {
    // Get file sizes from the server
    const fetchFileSizes = async () => {
      try {
        const promises = platforms.map(async (platform) => {
          const response = await fetch(`/api/file-size?file=${platform.file}`);
          if (response.ok) {
            const data = await response.json();
            return { file: platform.file, size: data.size };
          }
          // Fallback size if API fails
          return { file: platform.file, size: "~2.5 GB" };
        });
        
        const results = await Promise.all(promises);
        const sizes: Record<string, string> = {};
        results.forEach((result) => {
          sizes[result.file] = result.size;
        });
        ...
        platforms.forEach((platform) => {
          fallbackSizes[platform.file] = "~2.5 GB";
        });
        setFileSizes(fallbackSizes);
      }
    };
  ...
  }, []); // Remove platforms from the dependency array
                    ...
                    >
                      ...
                      <div className="p-8 text-center">
                        ...
                        <div className="text-gray-400 mb-6">
                          Size: {fileSizes[platform.file] || "Calculating..."}
                        </div>
                        <button
